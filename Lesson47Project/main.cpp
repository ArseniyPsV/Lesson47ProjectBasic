...
int main() {
	...
	for (int i = 0; i < count_showroom; i++)
	{
		do {
			...
			cin >> *(names + i);
			cout << "Input count of cars in " << *(names + i) << ": ";
			cin >> *(cars + i);
		} while (*(cars + i) <= 0);
	}
	...
	cout << convert(prices, count_showroom, names, cars);
	cout << "The rich showroom is " << *(names + find_index_of_rich_showroom(prices, count_showroom, cars)) << endl;
